...
def ensure_types_header_base():
    ...
    # 1. Clean out existing primitive blocks and SDK alias stubs
    ...
    # 2. Extract existing includes to move them below definitions
...
"""
    ...
    # Re-inject standard includes right after primitives
    if includes:
...
def apply_fixes(categories):
    ...
    for tag in ["Gfx", "Vtx", "Mtx"]:
        if tag in GBI_STRUCT_DEFINITIONS:
            # Check the current file content for the tag
    ...
    if isinstance(struct_bodies, (list, set, tuple)):
        ...
        for tag in sorted(struct_bodies):
            ...
            if body and not re.search(rf"\b{re.escape(tag)}\b", types_content):
                # Cleanup for Mtx to avoid union conflicts
                if tag == "Mtx":
