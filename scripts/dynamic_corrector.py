...
def apply_fixes():
    ...
    for filepath in affected_files:
        ...
        # FIX A: The 'close' Conflict (Renames internal functions to avoid POSIX clash)
        ...
        # FIX B: Intelligent Purifier (Only delete declarations WITHOUT bodies)
        for symbol in CORE_N64:
            # Matches 'typedef struct Name Name;' or 'extern Name name;' but NOT 'typedef struct { ... } Name;'
            ...
            if match and '{' not in match.group(0):
                ...
                print(f"  [🪠] Purged empty local declaration of {symbol}")
                fixes += 1

        # FIX C: Forward Declaration and Struct Movement
        # If 'LetterFloorTile' is unknown, we find it and move it to the top
        if "unknown type name 'LetterFloorTile'" in log_data and "typedef struct" in content:
            struct_match = re.search(r'(typedef struct \{.*?\}.*?LetterFloorTile;)', content, re.DOTALL)
            if struct_match:
                struct_def = struct_match.group(1)
                content = content.replace(struct_def, "")
                content = "/* Auto-moved to top */\n" + struct_def + "\n" + content
                print(f"  [⬆️] Moved LetterFloorTile definition to top of {os.path.basename(filepath)}")
                fixes += 1

        # FIX D: 'actor' pointer correction
        if "use of undeclared identifier 'actor'" in log_data and "this" in content:
            if "Actor *actor =" not in content:
                # Inject cast at the start of the function body
