...
def apply_fixes():
    ...
    CORE_N64 = {"__OSGlobalIntMask", "osClockRate", "osResetType", "osAppNMIBuffer", "OSThread", "CPUState"}
    ...
    for filepath in affected_files:
        ...
        if "follows non-static declaration" in log_data and "static" in content:
            content = re.sub(r'\bclose\b', 'bka_close', content)
            print(f"  [-] Resolved 'close' conflict in {os.path.basename(filepath)}")
            fixes += 1

        # FIX: Redefinition Purge
        for symbol in CORE_N64:
            pattern = rf"^(?:extern|volatile|static|typedef)?\s+[^;{{}}]+\s+{re.escape(symbol)}\b[^;{{}}]*;"
            if re.search(pattern, content, re.MULTILINE):
                content = re.sub(pattern, f"/* Purged local {symbol} */", content, flags=re.MULTILINE)
                fixes += 1

        # FIX: 'actor' pointer mismatch
        if "use of undeclared identifier 'actor'" in log_data and "this" in content:
            # Replaces actor usages with this or injects the pointer
        ...
        for identifier in missing_ids:
            if identifier.isupper() or identifier.startswith("CH_"):
                ...
                if macro:
                    ...
                    # Polyfill if unknown constant
                    if inject_to_header(f"#define {identifier} 0", identifier):
                        fixes += 1
...
def main():
    for i in range(1, 100):
        ...
        if run_build():
            print("\n✅ Build Successful!")
            return
        # Continue if ANY fixes were applied
        if apply_fixes() == 0:
