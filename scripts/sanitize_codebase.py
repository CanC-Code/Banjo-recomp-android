...
def is_modern_wrapper(filepath, content):
    ...
    if "/android/app/" in path_lower or "/jni/" in path_lower or "wrapper" in path_lower:
        return True
    if re.search(r'#\s*include\s*[<"]jni\.h[">]', content) or re.search(r'#\s*include\s*[<"]android/', content):
        return True
    return False
...
def inject_extern_c(content, filename):
    ...
    if 'extern "C"' in content or '#ifdef __cplusplus' in content: return content
...
def redirect_legacy_includes(content, headers_to_redirect, is_wrapper=False, filename=""):
    if filename not in CORE_TYPE_HEADERS:
        ...
        content = re.sub(r'#\s*include\s*[<"]PR/ultratypes\.h[">]', '/* Redirected */ #include <n64_types.h>', content)

    if not is_wrapper:
        for ch in headers_to_redirect:
            escaped_ch = ch.replace('.', r'\.')
            content = re.sub(rf'#\s*include\s*[<"]{escaped_ch}[">]', f'/* Redirected */ #include <n64_{ch}>', content)
    ...
    for header in sdk_headers:
        content = re.sub(rf'#\s*include\s*<(?![pP][rR]/){header}>', f'#include <PR/{header}>', content)
        content = re.sub(rf'#\s*include\s*"(?![pP][rR]/){header}"', f'#include "PR/{header}"', content)

    content = re.sub(r'#\s*include\s*[<"]PR/n_synth\.h[">]', '#include "n_synth.h"', content)
    content = re.sub(r'#\s*include\s*[<"]PR/n_synthInternals\.h[">]', '#include "n_synthInternals.h"', content)
    content = re.sub(r'#\s*include\s*[<"]PR/synthInternals\.h[">]', '#include "synthInternals.h"', content)
    content = re.sub(r'#\s*include\s*[<"]PR/n_libaudio_sn\.h[">]', '#include "n_libaudio_sn.h"', content)
