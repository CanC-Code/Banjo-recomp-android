...
class SourceConverter:
    def __init__(self, logic_dir="scripts/conversion_logic"):
        ...
        self.N64_OS_STRUCT_BODIES = {
            ...
            "OSDevMgr": """typedef struct OSDevMgr_s { int32_t active; struct OSThread_s *thread; struct OSMesgQueue_s *cmdQueue; struct OSMesgQueue_s *evtQueue; struct OSMesgQueue_s *acsQueue; int32_t (*dma)(int32_t, void*, void*, uint32_t); int32_t (*edma)(struct OSPiHandle_s*, int32_t, void*, void*, uint32_t); } OSDevMgr;""",
            ...
            "LetterFloorTile": """typedef struct LetterFloorTile_s { void *meshId; int state; float timeDeltaSum; } LetterFloorTile;""",
            "sChVegetable": "typedef struct sChVegetable_s { long long int force_align[64]; } sChVegetable;",
            "MapProgressFlagToDialogID": """typedef struct MapProgressFlagToDialogID_s { int16_t key; int16_t value; } MapProgressFlagToDialogID;""",
        }
    ...
    def _inject_primitives_block(self, content: str) -> str:
        ...
        # 2. Inject core missing types found in build log
        primitives = ""
        if "OSMesg" not in content:
            primitives += "typedef void *OSMesg;\n"
        if "OSTime" not in content:
            primitives += "typedef uint64_t OSTime;\n"
        if "n64_bool" not in content:
            primitives += "#ifndef n64_bool\n#define n64_bool int\n#endif\n"
        
        # 3. Handle shorthand types (u8, s32, etc)
        shorthands = {
            "u8": "uint8_t", "s8": "int8_t",
            "u16": "uint16_t", "s16": "int16_t",
            "u32": "uint32_t", "s32": "int32_t",
            "u64": "uint64_t", "s64": "int64_t",
            "f32": "float", "f64": "double"
        }
        for short, full in shorthands.items():
            if f" {short};" not in content and f"typedef {full} {short};" not in content:
                primitives += f"typedef {full} {short};\n"

        return content + "\n" + primitives

    def _handle_exceptasm_fixes(self, content: str) -> str:
        # Ensure external thread structures are linked correctly for exceptasm.cpp
        linkage_fix = (
            '#ifdef __cplusplus\n'
        ...
        # Accessing thread context via force_align for raw register manipulation
        content = re.sub(
            r'\(\s*\(\s*uint32_t\s*\*\s*\)\s*__osRunningThread->context\s*\)',
            '((uint32_t*)&__osRunningThread->context.force_align[0])',
            content
        )
        # Accessing registers via the named 'regs' struct
    ...
    def apply_to_file(self, file_path: str) -> int:
        ...
        if "n64_types.h" in file_path:
            # Clean up old definitions that might conflict
            tags_to_purge = [
                "OSTask", "OSScTask", "sChVegetable", "LetterFloorTile", 
                "MapProgressFlagToDialogID", "uint32_t", "uint64_t", "OSMesg"
            ]
            for tag in tags_to_purge:
                content = re.sub(rf'typedef\s+struct\s+{tag}_s\s+.*?;?\n?', '', content)
                content = re.sub(rf'struct\s+{tag}_s;\n?', '', content)

            content = self._inject_primitives_block(content)

            # Inject all OS structs
