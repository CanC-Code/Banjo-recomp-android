...
def _scrub_linkage_comments(content: str) -> str:
    ...
    for i, line in enumerate(lines):
        if "AUTO-FIX LINKAGE:" in line:
            # Annihilate all permutations of the macro wrapper generated by past runs
...
def heal_corrupted_headers():
    """Proactively scour header directories to repair syntax destruction in files like gu.h before evaluating new bounds"""
    search_dirs = [
...
def strip_redefinition(content: str, tag: str) -> str:
    """Robust parser that strips struct/union definitions via bracket evaluation and suffix variant checks."""
    
    # Pass 1: Eliminate macro guards that silently obscure the redefined block
    content = re.sub(rf"(?m)^\s*#\s*define\s+{re.escape(tag)}(?:_s|_t|_n|_u)?_DEFINED\b.*$", f"/* STRIPPED DEFINE: {tag}_DEFINED */", content)
    
    # Pass 2: Clear compact inline forward structs 
    content = re.sub(rf"(?m)^\s*typedef\s+(?:struct|union)\s+[A-Za-z0-9_]+\s+{re.escape(tag)}\s*;\s*$", f"/* STRIPPED FWD: {tag} */", content)
    
    # Pass 3: State-machine syntax evaluation for bracketed structs accommodating underlying N64 SDK suffixes (Tag_s)
    ...
    while True:
        ...
        if semi_idx != -1 and '{' not in content[curr_idx:semi_idx]:
            ...
            # Annihilate the structure if the core tag or its suffixed equivalent is discovered 
    ...
    # Pass 4: Final sweep for dangling legacy forwards 
...
def _scrape_logs_into_categories(categories: dict) -> None:
    log_candidates = ["Android/full_build_log.txt", "full_build_log.txt", "build_log.txt", "Android/failed_files.log"]
...
def apply_fixes(categories: dict, intelligence_level: int = 3) -> Tuple[int, set]:
    ...
    # Preemptively heal the recursion syntax failure hidden in standard headers before we proceed
    ...
    # Level 1 primitives inject first to cascade visibility into complex wrappers down the line. 
    ORDERED_STRUCT_TAGS = [
        # Primitives
    ...
    # ------------------------------------------------------------------
    # PHASE 3: LINKAGE FIX (Gu.h Recursive Clean up)
    # ------------------------------------------------------------------
